...
export default class HomeTemplate {
  ...
  readonly setHome = (home: Home) => {
    this.home = home
  }

  readonly getHomeNode = (): HTMLDivElement => {
    ...
    container.innerHTML = `
      ...
      <div class="home-links">
        ${this.home.quickLinks.map(link => 
          `<button class="quick-link" data-action="${link.action}">${link.text}</button>`
        ).join('')}
      </div>
    `
